...
class FakeHomeNotifier extends BaseUiNotifier<HomeState> implements HomeNotifier {
  FakeHomeNotifier() {
...
class FakeSearchNotifier extends BaseUiNotifier<SearchState> implements SearchNotifier {
  FakeSearchNotifier() {
...
class FakeNotificationNotifier extends BaseUiNotifier<NotificationState> implements NotificationNotifier {
...
class FakeAnalyticsNotifier extends BaseUiNotifier<AnalyticsState> implements AnalyticsNotifier {
...
class FakeTransactionsNotifier extends BaseUiNotifier<TransactionsState> implements TransactionsNotifier {
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockPrefs.getBool(any())).thenAnswer((_) => SynchronousFuture(false));
    when(() => mockPrefs.getBool('by_onboarding_completed')).thenAnswer((_) => SynchronousFuture(true));
    when(() => mockPrefs.getString(any())).thenAnswer((_) => SynchronousFuture(null));
    when(() => mockPrefs.setBool(any(), any())).thenAnswer((_) => SynchronousFuture(null));
    when(() => mockPrefs.setString(any(), any())).thenAnswer((_) => SynchronousFuture(null));

    // Platform engine mocks
    when(() => mockImporter.performIncrementalSync()).thenAnswer((_) => SynchronousFuture(0));
    when(() => mockReceiver.startListening()).thenAnswer((_) => SynchronousFuture(null));

    // Transaction Repository Mocking
    when(() => mockTxRepository.getTransactionsPaged(
    ...
    when(() => mockTxRepository.getCategories()).thenAnswer((_) => SynchronousFuture(const Result.success([])));
    when(() => mockTxRepository.getTransactions()).thenAnswer((_) => SynchronousFuture(const Result.success([])));
    when(() => mockTxRepository.watchTransactions()).thenAnswer((_) => const Stream.empty());
  ...
  group('DataManagementNotifier Ingestion & Deletion Unit Tests', () {
    test('startHistoricalImport correctly manages scanning state and handles empty rawMessages', () async {
      final container = ProviderContainer(
        overrides: [
          ...
          notificationNotifierProvider.overrideWith((ref) => fakeNotificationNotifier),
          analyticsViewModelProvider.overrideWith((ref) => fakeAnalyticsNotifier),
          homeViewModelProvider.overrideWith((ref) => fakeHomeNotifier),
          transactionsViewModelProvider.overrideWith((ref) => fakeTransactionsNotifier),
        ],
      );

      final notifier = container.read(dataManagementNotifierProvider.notifier);

      expect(container.read(dataManagementNotifierProvider).isImporting, isFalse);

      final importFuture = notifier.startHistoricalImport(range: ImportRange.all);
      expect(container.read(dataManagementNotifierProvider).isImporting, isTrue);
    ...
    test('startHistoricalImport processes messages, checks duplicates, and executes batch database commits', () async {
      final container = ProviderContainer(
        overrides: [
          databaseServiceProvider.overrideWithValue(mockDbService),
          preferencesStorageProvider.overrideWithValue(mockPrefs),
      ...
      when(() => mockDb.transaction<Null>(any())).thenAnswer((inv) async {
        final Function transactionCallback = inv.positionalArguments[0] as Function;
      ...
      expect(state.successMessage, contains('وارد کردن پیامک‌های قبلی به پایان رسید.'));

      // Verify batch insert call
      verify(() => mockBatch.insert('bank_messages', any(), conflictAlgorithm: any(named: 'conflictAlgorithm'))).called(1);
      verify(() => mockBatch.insert('transactions', any(), conflictAlgorithm: any(named: 'conflictAlgorithm'))).called(1);
      verify(() => mockBatch.commit(noResult: true)).called(1);
    });

    test('startHistoricalImport handles selection range options and filters correct bounds', () async {
      final container = ProviderContainer(
        overrides: [
          databaseServiceProvider.overrideWithValue(mockDbService),
          preferencesStorageProvider.overrideWithValue(mockPrefs),
      ...
      final threeMonthsAgo = DateTime.now().subtract(const Duration(days: 90)).millisecondsSinceEpoch;
      expect((capturedSinceTimestamp - threeMonthsAgo).abs(), lessThan(5000));
    });

    test('Bulk delete operations execute atomically and return success', () async {
      final container = ProviderContainer(
        overrides: [
          databaseServiceProvider.overrideWithValue(mockDbService),
          preferencesStorageProvider.overrideWithValue(mockPrefs),
      ...
      when(() => mockTxn.delete(any(), where: any(named: 'where'), whereArgs: any(named: 'whereArgs'))).thenAnswer((_) async => 1);
      when(() => mockDbService.wipeLocalSandboxData()).thenAnswer((_) async => const Success<void>(null));
  ...
  group('HomeScreen & SecurityDashboardScreen Integration Widget Tests', () {
    testWidgets('HomeScreen triggers historical SMS prompt when SMS read permission is granted', (tester) async {
      final container = ProviderContainer(
        overrides: [
          databaseServiceProvider.overrideWithValue(mockDbService),
          preferencesStorageProvider.overrideWithValue(mockPrefs),
      ...
      when(() => mockPrefs.getBool('by_historical_sms_import_offered')).thenAnswer((_) => SynchronousFuture(false));
      ...
      expect(find.text('بانک‌یار می‌تواند پیامک‌های بانکی قبلی شما را نیز بررسی و وارد کند.'), findsOneWidget);
    ...
    testWidgets('SecurityDashboardScreen renders Data Management card and prompts destructive warning', (tester) async {
      final container = ProviderContainer(
        overrides: [
          databaseServiceProvider.overrideWithValue(mockDbService),
          preferencesStorageProvider.overrideWithValue(mockPrefs),
