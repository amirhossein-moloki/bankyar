...
void main() {
  ...
  group('DeveloperExportService Unit Tests', () {
    test(
      'generateJsonExport produces schema-compliant pretty-printed JSON with preserved Persian chars',
      () async {
        // Stub tag relations query
        when(
          () => mockDb.rawQuery(any(that: contains('transaction_tags'))),
        ).thenAnswer(
          (_) async => [
        ...
        when(
          () => mockDb.rawQuery(any(that: contains('LEFT JOIN notes'))),
        ).thenAnswer(
          (_) async => [
            {
              'id': 'tx-1',
  ...
  group('JSON Export Interaction & Widget Tests', () {
    testWidgets(
      'Export button appears on HomeScreen when developer mode is enabled and triggers SAF method call',
      (WidgetTester tester) async {
        // Enable developer mode in mock preferences
        when(
          () => mockPrefs.getBool('by_developer_mode_enabled'),
        ).thenAnswer((_) async => true);
        ...
        final iconButton = tester.widget<IconButton>(
          find.ancestor(of: exportBtnFinder, matching: find.byType(IconButton)),
        );
        ...
        final exportCall = methodCalls.firstWhere(
          (call) => call.method == 'exportJsonViaSAF',
        );
        ...
        expect(
          exportCall.arguments['content'],
          contains('"export_version": 1'),
        );
