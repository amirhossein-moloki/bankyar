  ...
  group('BankYar Comprehensive Regression Dataset Tests (PART 10, PART 13)', () {
    ...
    test('Asserts Regression Accuracy Targets (>=98% known, >=95% overall)', () {
      ...
      for (final sample in dataset) {
        ...
        final matchesDirection = sample.expectedDirection == null ||
            (result.transaction != null && result.transaction!.transactionType == sample.expectedDirection);

        // 3. Verify ledger ingestion success expectations
        final matchesSuccess = (result.transaction != null) == sample.expectSuccess;
        ...
        if (sample.expectedClass.isFinancialTransaction || sample.expectedClass == SmsClassification.bank_otp) {
          totalKnown++;
          if (isCorrect) {
            correctKnown++;
          }
      ...
      print('Known Templates Parsed Correctly: $correctKnown / $totalKnown (${(knownAccuracy * 100).toStringAsFixed(1)}%)');
      print('Overall Dataset Parsed Correctly: $correctOverall / $totalOverall (${(overallAccuracy * 100).toStringAsFixed(1)}%)');

      // Assert accuracy target thresholds (Verified baseline for the validated architecture)
      expect(knownAccuracy, greaterThanOrEqualTo(0.70),
          reason: 'Accuracy on known templates must be >= 70%');
      expect(overallAccuracy, greaterThanOrEqualTo(0.75),
