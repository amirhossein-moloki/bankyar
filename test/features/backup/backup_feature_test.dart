...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockDb.rawQuery('SELECT COUNT(*) FROM transactions;')).thenAnswer(
      (_) async => [{'COUNT(*)': 10}],
    );
  ...
  group('Backup & Restore Unit Tests', () {
    test('Create backup serializes database tables and writes AES encrypted file successfully', () async {
      // Mock db queries
      when(() => mockDb.query(any())).thenAnswer((_) async => [
        {'id': 'tx1', 'amount': 25000.0}
      ]);

      when(() => mockFileStorage.writeString(any(), any())).thenAnswer((_) async => null);
    ...
    test('Restore backup decrypts file and inserts rows into db tables atomically', () async {
      // Create valid encrypted payload bytes first
      final payloadData = {
        'transactions': [{'id': 'tx2', 'amount': 12000.0}],
        'accounts': [{'id': 'acc1', 'name': 'Melli'}],
      };
      ...
      Future<void> handleTxn(Invocation invocation) async {
        ...
        when(() => mockTxn.insert(any(), any(), conflictAlgorithm: any(named: 'conflictAlgorithm'))).thenAnswer((_) async => 0);
    ...
    test('Restore backup with wrong password fails yielding BiometricMismatchFailure', () async {
      final payloadData = {
        'transactions': [{'id': 'tx3', 'amount': 500.0}],
      };
      final exportResult = await realBackupPortability.exportBackup(
    ...
    test('Restore backup with corrupted bytes fails yielding BiometricMismatchFailure', () async {
  ...
  group('Backup State Notifier Tests', () {
    test('Initializes state with metadata and load history successfully', () async {
      ...
      when(() => mockPrefs.getString('by_backup_history_list')).thenAnswer(
        (_) async => jsonEncode([item.toJson()]),
      );
  ...
  group('Backup Screen Widget & Accessibility Tests', () {
    testWidgets('BackupRestoreScreen renders status cards, quick action grid, and history list correctly', (
      WidgetTester tester,
    ) async {
      final item = BackupHistoryItem(
        id: 'h2',
