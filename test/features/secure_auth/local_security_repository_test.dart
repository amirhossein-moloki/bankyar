...
void main() {
  ...
  group('LocalSecurityRepository Tests', () {
    test('isPinConfigured returns true when hash is stored', () async {
      when(
        () => secureStorage.read('by_sec_pin_hash'),
    ...
    test(
      'savePin saves hashed pin and salt to secure storage and sets pin enabled preference',
      () async {
        when(
          () => secureStorage.write(
            key: any(named: 'key'),
        ...
        when(
          () => preferencesStorage.setBool('by_sec_pin_enabled', true),
    ...
    test('verifyPin returns true on correct matching hash', () async {
      ...
      when(
        () => secureStorage.read('by_sec_pin_salt'),
    ...
    test('changePin returns failure on wrong old PIN', () async {
      when(
        () => secureStorage.read('by_sec_pin_salt'),
      ).thenAnswer((_) async => 'salt123');
      when(
        () => secureStorage.read('by_sec_pin_hash'),
      ).thenAnswer((_) async => 'some_wrong_hash');
    ...
    test('getSettings retrieves correct settings', () async {
      when(
        () => preferencesStorage.getBool('by_sec_pin_enabled'),
      ).thenAnswer((_) async => true);
      when(
        () => preferencesStorage.getBool('by_sec_biometrics_enabled'),
      ).thenAnswer((_) async => false);
      when(
        () => preferencesStorage.getInt('by_sec_auto_lock_timeout'),
      ).thenAnswer((_) async => 30);
      when(
        () => preferencesStorage.getBool('by_sec_privacy_mode_enabled'),
      ).thenAnswer((_) async => true);
    ...
    test(
      'updateSettings writes to PreferencesStorage and updates by_balance_obscured',
      () async {
        when(
          () => preferencesStorage.setBool('by_sec_pin_enabled', true),
        ).thenAnswer((_) async => true);
        when(
          () => preferencesStorage.setBool('by_sec_biometrics_enabled', true),
        ).thenAnswer((_) async => true);
        when(
          () => preferencesStorage.setInt('by_sec_auto_lock_timeout', 60),
        ).thenAnswer((_) async => true);
        when(
          () => preferencesStorage.setBool('by_sec_privacy_mode_enabled', true),
        ).thenAnswer((_) async => true);
        when(
          () => preferencesStorage.setBool('by_balance_obscured', true),
        ).thenAnswer((_) async => true);
    ...
    test(
      'getBiometricCapabilities returns correct hardware availability and enrollment status',
      () async {
        when(
          () => permissionService.checkStatus(AppPermission.biometrics),
    ...
    test(
      'authenticateBiometrics returns true when simulation succeeds',
      () async {
        when(
          () => permissionService.checkStatus(AppPermission.biometrics),
        ).thenAnswer((_) async => PermissionStatus.granted);
    ...
    test(
      'authenticateBiometrics returns failure when simulation fails',
      () async {
        when(
          () => permissionService.checkStatus(AppPermission.biometrics),
        ).thenAnswer((_) async => PermissionStatus.granted);
    ...
    test('getSession loads correct SessionModel', () async {
      when(
        () => preferencesStorage.getBool('by_sec_session_auth'),
      ).thenAnswer((_) async => true);
      when(
        () => preferencesStorage.getInt('by_sec_session_failed_attempts'),
      ).thenAnswer((_) async => 2);
      when(
        () => preferencesStorage.getString('by_sec_session_lockout_until'),
      ).thenAnswer((_) async => null);
    ...
    test('saveSession writes current session properties', () async {
      when(
        () => preferencesStorage.setBool('by_sec_session_auth', true),
      ).thenAnswer((_) async => true);
      when(
        () => preferencesStorage.setInt('by_sec_session_failed_attempts', 0),
      ).thenAnswer((_) async => true);
      when(
        () =>
            preferencesStorage.setString('by_sec_session_lockout_until', any()),
      ).thenAnswer((_) async => true);
    ...
    test(
      'purgeAllData wipes local database, secure storage and preferences',
      () async {
        when(
          () => databaseService.wipeLocalSandboxData(),
