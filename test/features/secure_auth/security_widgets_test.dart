...
void main() {
  ...
  Widget createTestWidget(Widget child, List<Override> overrides) {
    return ProviderScope(
      overrides: overrides,
      child: MaterialApp(theme: AppTheme.createThemeLight(), home: child),
    );
  }

  group('PinKeypad Widget Tests', () {
    testWidgets(
      'Renders all digits and accessories correctly with semantic labels',
      (tester) async {
        String? tappedDigit;
        ...
        await tester.pumpWidget(
          MaterialApp(
            theme:
                AppTheme.createThemeLight(), // Configures SpacingExtension correctly
            home: Scaffold(
              body: PinKeypad(
  ...
  group('UnlockScreen Tests', () {
    testWidgets(
      'Renders PIN title, dots, and triggers biometrics verification on scan tap',
      (tester) async {
        final overrides = [
        ...
        await tester.pumpWidget(
          createTestWidget(const UnlockScreen(), overrides),
        );
    ...
    testWidgets(
      'Renders 12-Word support recovery layout when forgot PIN tapped',
      (tester) async {
  ...
  group('SecurityDashboardScreen Tests', () {
    testWidgets(
      'Renders security score, device trust, toggles, and emergency hold actions',
      (tester) async {
        final overrides = [
          clockProvider.overrideWithValue(mockClock),
          permissionServiceProvider.overrideWithValue(mockPermissionService),
        ...
        await tester.pumpWidget(
          createTestWidget(const SecurityDashboardScreen(), overrides),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          find.text('بخش حریم خصوصی و عدم دسترسی اینترنت'),
          findsOneWidget,
        );
