...
class MockVerifyBiometricsUseCase extends Mock
    implements VerifyBiometricsUseCase {}

class MockLocalSecurityRepository extends Mock
    implements LocalSecurityRepository {}
...
void main() {
  ...
  setUpAll(() {
    ...
    registerFallbackValue(
      const SessionModel(isAuthenticated: false, failedAttempts: 0),
    );
  ...
  setUp(() async {
    ...
    when(() => securityRepository.getSession()).thenAnswer(
    ...
    when(() => securityRepository.getSettings()).thenAnswer(
      (_) async => const Result.success(
        SecuritySettings(
          isPinEnabled: true,
    ...
    when(() => securityRepository.getBiometricCapabilities()).thenAnswer(
      (_) async => const Result.success(
    ...
    when(
      () => securityRepository.saveSession(any()),
    ...
    container = ProviderContainer(
      overrides: [
        ...
        verifyBiometricsUseCaseProvider.overrideWithValue(
          verifyBiometricsUseCase,
        ),
      ],
    );
  ...
  group('AppLockCoordinator Tests', () {
    test(
      'Initializes correctly with closed/unlocked status depending on PIN preference',
      () async {
    ...
    test('appendDigit appends digits up to 4 and auto-verifies', () async {
      ...
      expect(
        container.read(appLockCoordinatorProvider).currentInputPin,
        equals('1'),
      );
    ...
    test(
      'Increments fail attempts and triggers 60s doubling lockout cooldown on wrong PIN >= 3',
      () async {
        when(
          () => verifyPinUseCase(any()),
        ...
        await Future<void>.delayed(const Duration(milliseconds: 10));
        expect(
          container.read(appLockCoordinatorProvider).session.failedAttempts,
          equals(1),
        );
        expect(
          container.read(appLockCoordinatorProvider).session.isLockedOut,
          isFalse,
        );
        ...
        expect(
          ...
          equals(DateTime(2023, 10, 1, 12, 1, 0)),
        );
      },
    );
    ...
    test('Permanent lockout is triggered after 15 failed attempts', () async {
      ...
      final coordinator = customContainer.read(
        appLockCoordinatorProvider.notifier,
      );
    ...
    test(
      '12-Word seed recovery resets PIN protection and unlocks session',
      () async {
        final coordinator = container.read(appLockCoordinatorProvider.notifier);
        when(
          () => securityRepository.updateSettings(any()),
        ).thenAnswer((_) async => const Result.success(null));
    ...
    test(
      'Auto Lock Timeout triggers locking when background elapsed duration exceeds settings',
      () async {
        final coordinator = container.read(appLockCoordinatorProvider.notifier);
        ...
        await Future<void>.delayed(const Duration(milliseconds: 10));
        expect(
          container.read(appLockCoordinatorProvider).isAppUnlocked,
          isTrue,
        );
    ...
    test(
      'Auto Lock Timeout does NOT trigger lock when elapsed background duration is within timeout',
      () async {
        final coordinator = container.read(appLockCoordinatorProvider.notifier);
