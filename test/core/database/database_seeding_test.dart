...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockLogger.log(any(), any(), any(), any(),
  ...
  group('DatabaseServiceImpl Account Seeding Tests', () {
    test('seeds standard accounts when accounts table is empty', () async {
      ...
      when(() => mockDb.rawQuery('SELECT COUNT(*) as cnt FROM accounts'))
          .thenAnswer((_) async => [
                {'cnt': 0}
              ]);
      ...
      verify(() => mockDb.rawQuery('SELECT COUNT(*) as cnt FROM accounts'))
          .called(1);
      verify(() => mockDb.batch()).called(1);

      // 5. Verify standard 7 bank profiles are inserted
      final captured = verify(() => mockBatch.insert(
            captureAny(),
            captureAny(),
            conflictAlgorithm: ConflictAlgorithm.ignore,
          )).captured;

      expect(captured, hasLength(14)); // 7 calls * 2 captured arguments per call
